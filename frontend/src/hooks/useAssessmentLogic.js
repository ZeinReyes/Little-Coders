...
export function useAssessmentLogic({ lessonId, user }) {
  ...
  const fetchSuggestedDifficulty = async (updatedHistory, currentDifficulty, remaining) => {
    try {
      ...
      const res = await axios.post(
        `https://little-coders-production.up.railway.app/api/ai/suggest-difficulty`,
        {
  ...
  const markAssessmentCompleted = async (assessmentId, isAIReview, currentHistory) => {
    ...
    try {
      ...
      if (history.length >= 2) {
        await axios.post(
          `https://little-coders-production.up.railway.app/api/ai/record-session`,
      ...
      await axios.post(
        `https://little-coders-production.up.railway.app/api/progress/complete-assessment`,
